...
DesertClient::DesertClient(std::string service_name, const rosidl_service_type_support_t * type_supports, rmw_gid_t gid)
      ...
      , _request_data_stream(dccl::TxStream(CLIENT_TYPE, service_name, _id, ProtobufHelper::rosidl_to_proto(_service, CLIENT_TYPE, _c_cpp_identifier, _id)))
      , _response_data_stream(dccl::RxStream(CLIENT_TYPE, service_name, _id, ProtobufHelper::rosidl_to_proto(_service, SERVICE_TYPE, _c_cpp_identifier, _id)))
{
}
...
bool DesertClient::has_data()
{
  ...
  _response_data_stream.interpret_packets(_sequence_id);
  return _response_data_stream.data_available();
}
