...
{
  cbor_writer_t libmcu_writer;
};

struct TxStream::ERROR
{
  cbor_error_t libmcu_error;
};
...
void TxStream::new_packet()
{
  ...
  cbor_writer_init(&_writer->libmcu_writer, _packet, MAX_PACKET_LENGTH);
}
...
void TxStream::end_transmission()
{
  ...
  for(size_t i=0; i < cbor_writer_len(&_writer->libmcu_writer); i++)
  {
...
{
  cbor_error_t result = cbor_encode_unsigned_integer(&_writer->libmcu_writer, n);
...
TxStream & TxStream::operator<<(const int64_t n)
{
  ...
  if (n >= 0)
    result = cbor_encode_unsigned_integer(&_writer->libmcu_writer, n);
  else
    result = cbor_encode_negative_integer(&_writer->libmcu_writer, n);
...
{
  cbor_error_t result = cbor_encode_float(&_writer->libmcu_writer, f);
  handle_overrun(ERROR(result));
  return *this;
}
...
{
  cbor_error_t result = cbor_encode_text_string(&_writer->libmcu_writer, s.c_str(), s.size());
  handle_overrun(ERROR(result));
  return *this;
}
...
{
  cbor_error_t result = cbor_encode_bool(&_writer->libmcu_writer, b);
  handle_overrun(ERROR(result));
  return *this;
}
...
{
  if (result.libmcu_error == CBOR_OVERRUN)
  {
    _overflow = true;
  }
}
...
void RxStream::interpret_packets()
{
  ...
  for (packet = TcpDaemon::read_packet(); packet.size() != 0; packet = TcpDaemon::read_packet())
  {
    ...
    cbor_reader_init(&reader, &items->libmcu_item, sizeof(items) / sizeof(items[0]));
    ...
    for (size_t i = 0; i < n; i++)
    {
      ...
      cbor_decode(&reader, &items[i].libmcu_item, &val, sizeof(val));
...
std::pair<void *, int> RxStream::interpret_field(ITEM * items, size_t i, union _cbor_value & val)
{
  switch (items[i].libmcu_item.type)
  {
    ...
    {
      val.str_copy[items[i].libmcu_item.size] = '\0';
